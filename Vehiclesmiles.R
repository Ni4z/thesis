...
###############################################################################
# Import Data
new_data <- read_xls("VehiclesMiles.xls")
...
autoplot.zoo(Xt_sa_zoo) +
  ...
  ggtitle("Vehicle Miles - Seasonally Adjusted Monthly Data from 2000 to 2019")
...
autoplot.zoo(sa_data_zoo, facets = NULL) +
  ...
  xlab("Year") + 
  ylab("Data (SA)") +
  ggtitle("Vehicle MIles - Seasonally Adjusted Data with LM & SM Trends") +
  scale_color_manual(
    name   = "Series",
...
autoplot.zoo(farima_res) +
  ...
  xlab("Year") + 
  ylab("Residuals") +
  ggtitle("Vehicle Miles - FARIMA Residuals (LM Approach on SA Data)")
...
farima_fit <- fracdiff(res_lm, nar = est_lm$p.BIC, nma = est_lm$q.BIC, h = 0.01)
...
ggplot(.df, aes(x = Year, y = Residuals)) +
  ...
  ylab("Residuals") +
  ggtitle("Vehicles Miles Driven - Residual Series (SM vs LM)") +
  facet_grid(Model ~ .)
